...
interface IDVP is IDVPImmutables, IDVPEvents {
    ...
    /// @return entryPrice The entry price of the position
    function positions(
        bytes32 key
    ) external view returns (uint256 amount, uint256 strategy, uint256 entryPrice, uint256 epoch);
    ...
    function mint(address recipient, uint256 strategy, uint256 amount) external;

    /// @notice Burns an option transferring back the payoff to the owner
    /// TODO
    function burn(address recipient, uint256 strategy, uint256 amount) external;
}
